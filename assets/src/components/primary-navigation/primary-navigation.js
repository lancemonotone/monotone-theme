...
window.addEventListener('DOMContentLoaded', (event) => {
	const menu = document.querySelector('.target-menu-wrapper');
	if (!menu) {
	...
	let startOfNewColumn2 = [];
	let splitTerm, splitTerm2;

    // Check if menu has 'full-primary-menu-container' class and assign values accordingly
    if (menu.classList.contains('primary-menu-wrapper')) {
        splitTerm = ['Training & Research', 'Membership & Support'];
        splitTerm2 = ['About Monotone'];
    } else {
        splitTerm = ['Help by State', 'Counselor Directory'];
		splitTerm2 = ['Choosing the Right Treatment Program'];
    }
	...
	for (let i = 0; i < splitTerm2.length; i++) {
		const index2 = items.findIndex(item => item.textContent.includes(splitTerm2[i]));
		startOfNewColumn2.push(index2);
	}
	...
	if (screenWidth >= 1040 && startOfNewColumn.length === 2) {
		...
		const column1 = createColumnWrapper(0, startOfNewColumn2[0]);
		const column2 = createColumnWrapper(startOfNewColumn2[0], items.length);
		menu.appendChild(column1);
		menu.appendChild(column2);
	} else {
		// one column wrapper
	...
	// toggle the corporate navigation helpline popup addition open and closed
	const helplineToggleButton = document.querySelector('.helpline-corp-nav-wrapper .icon-wrap');
    const helplineContainer = document.querySelector('.helpline-corp-nav-wrapper');
    const helplineContainerInner = document.querySelector('.helpline-corp-nav-wrapper .bottom-part');

    if (!helplineToggleButton) {
		// If there's no menu, none of this is necessary.
		return;
	}

    helplineToggleButton.addEventListener('click', function() {
        if (helplineContainer.classList.contains('open')) {
            slideUp(helplineContainerInner); // Slide up if open
        } else {
            slideDown(helplineContainerInner); // Slide down if not open
        }

        helplineContainer.classList.toggle('open');
    });
});
